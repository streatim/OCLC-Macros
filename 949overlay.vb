'MacroName:949overlay
'MacroDescription:949overlay
sub main
  Dim CS as object
...
skipcall:
   ...
   locs=InputBox$("Enter location:","949 Overlay",callloc)
   if locs="" then goto out
ovi:  ovcod=InputBox$("Enter Bib record number to overlay"&chr$(10)&"(.b is not necessary)","949 Overlay")
   if ovcod="" then goto out
   if len(ovcod)<>8 then
   goto ovi:
   end if
bci:  barcode=InputBox$("Scan barcode:","949 Overlay")
   if barcode="" then goto out
   if len(barcode)<>14 then
   goto bci:
   end if
    tycod=InputBox$("Enter TY code:","949 Overlay","0")
...
make949:
CS.Addfieldline 1,"949  *recs=b;bn=" & locs & ";ov=.b" & ovcod & ";ins=" & userinit & ";i=" & barcode & "/sta=" & istat & "/loc=" & locs & "/ty=" & tycod & "/i2=" & icode2 & "/b2=" & formatcode & ";"
